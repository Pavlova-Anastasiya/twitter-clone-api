...
- регистрироваться (авто-создание по `api-key`);
...
- **Фронтенд (готовый twitter-clone на Vue)** — одностраничное приложение, которое общается с бэкендом через HTTP-запросы.

Авторизация реализована через заголовок `api-key`:
- при первом запросе с новым `api-key` бэкенд автоматически создаёт пользователя с этим ключом;
- далее все запросы от фронтенда к API выполняются от имени этого пользователя.

Через web-интерфейс пользователь может:
- создавать твиты (с текстом и медиа),
- просматривать свою ленту подписок,
